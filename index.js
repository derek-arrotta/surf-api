...
// FETCH DATA FROM API's
  function getCurrentDate() {
    ...
    for (let i=20; i<22; i++) {
      timeFormatted4 += timeUTCiso[i];
    }
    console.log(timeFormatted1);
    console.log(timeFormatted2);
    console.log(timeFormatted3);
    console.log(timeFormatted4);
    const timeFormatted = timeFormatted1 + '%3A' + timeFormatted2 + '%3A' + timeFormatted3 + '%2B' + timeFormatted4 + '%3A00';
  ...
  function getSgData() {
    // stormglass.io
    const lat = 43.1694497; // Lake Harbor Park
    ...
    const start = timeFormatted;
    const end = timeFormatted;
  ...
  function displaySgData(jsonSgData) {
    ...
    for (let i=0; i<jsonSgData.hours.length; i++) {
      $('#results-list').append(
        `<h2>Surf Location</h2>
        <li>
        <p>${jsonSgData.hours[i].windDirection.sg}</p>
        <p>${jsonSgData.hours[i].windSpeed.sg}</p>
        <p>${jsonSgData.hours[i].swellHeight.sg}</p>
        <p>${jsonSgData.hours[i].airTemperature.sg}</p>
        <p>${jsonSgData.hours[i].waterTemperature.sg}</p>
        </li>`
    )};
